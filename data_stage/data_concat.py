...
def process_subdir(subdir, output_dir):
...
class Data_Concat:
    ...
    def gather_files(self):
        ...
        :param output_dir: pdf 파일을 모을 디렉토리
        """
        # SparkSession 생성
        spark = SparkSession.builder \
            .appName("Gather PDF Files") \
            .getOrCreate()
        ...
        try:
            ...
            # Spark RDD 생성
            rdd = spark.sparkContext.parallelize(subdirs)

            # RDD 변환 및 처리
            results = rdd.flatMap(lambda subdir: process_subdir(subdir, self.output_dir)).collect()

            # 결과 출력
            if not results:
                logger.info("No PDF files were copied.")
            else:
                logger.info(f"Processing complete. Total PDF files copied: {len(results)}")

        except Exception as e:
            logger.exception(f"An error occurred while processing directories with Spark: {e}")
        finally:
            spark.stop()